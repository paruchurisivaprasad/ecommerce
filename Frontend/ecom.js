...
let purchaseitem = document.querySelector(".purchaseItem");
 ...
 axios.get("http://localhost:1111/allcart")
  .then(items=>{
    ...
    for(let i=0;i<items.data.length;i++){
      ...
      productid+=j++ +') '+ items.data[i].productname+"  ";
    }
    ...
    axios.post("http://localhost:1111/ordernow",ob)
    .then(result=>{
if(result.data.msg){
  axios.delete("http://localhost:1111/deleteallcart")
  .then(result=>{
  getallcartitems();
  })
  .catch(err=>{
